...
SAVE_BUFFERS = int(0 * 44100 / 2048)  # set to 0 for no saving
SAVE_DIR = "debug"

class AudioPublisher(NodeWithParams):
    PARAMS_DICT = {"n_buffer": N_BUFFER}

    def __init__(self, name="audio_publisher", publish_rate=None, Fs=None, mic_positions=None):
        super().__init__(name)
    ...
    def process_signals(self, signals):
        ...
        #########threshold###########
        #signals[np.logical_not(np.isfinite(signals))] = 0.0
        threshold = 0.1  # Adjust this threshold according to your environment
        max_amplitude = np.max(np.abs(signals))
        #print(max_amplitude)
        if max_amplitude < threshold:
            return
            #signals = np.zeros_like(signals)  # Set the signals to zero to ignore small amplitudes
        print(signals)
        #############################
        ...
        if SAVE_BUFFERS > 0:
            if self.save_data is None:
                self.save_data = np.empty((1, n_buffer * SAVE_BUFFERS))
