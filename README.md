...
- [기획서 작성 및 팀원 모집](https://docs.google.com/presentation/d/1avigauiQwAB-vyEpPH3pFnGLdhsvMhWZs4tUgz76kmw/edit?usp=sharing)
...
-현재 음성 송수신 저장 품질이 만족스럽지 못해 태웅씨가 음성처리 패키지를 개선해서 다시 만들어보겠다고 한다. 
...
- 또한 미니봇이 가능한 동작들을 생각하고 최대한 제작한다.
...
- 또한 받아들일 정보가 텍스트밖에 존재하지 않는 GPT 모델을 위해서 YOLO 등 외부 정보를 넣어줄 수 있는 길을 만들어주고, 인터넷이나 로봇의 상태, 현재 상황 등의 정보를 넣어줄 필요가 있다.
이는 GPT Feedback이라는 토픽으로 구현하였으며, 모델이 특정함수를 호출하면 사용되도록 한다.  

### 2023.6.24
- 완성된 로봇팔과 미니봇을 소프트웨어적으로 통합하는 과정.
두 하드웨어가 같은 이름의 joint등의 토픽을 공유하는 탓인지 같은 ROS DOMAIN ID 상에서 동시에 bringup 시키면 터지고 만다.
이를 뜯어고치기엔 시간적 여유가 부족하다.
- 텍스트 파일로 중계하여 두 ROS DOMAIN ID 사이의 정보를 교환하는 노드를 만든다. 
- 동시에 bringup되고 정보가 잘 교환되어 두 기체가 한 몸처럼 잘 동작하는 것을 확인했다.

### 2023.6.25
- finetuning 데이터 제작.
- 이전의 데이터는 기기를 지정하지 않았으나 이번 데이터는 기기 정보도 문법에 들어가도록 하며, 한글을 사용하던 이전 데이터와 달리 이번 데이터는 google 영어 STT TTS API를 사용해야 하기에 영어를 사용한다.
- OpenAI에서 제공하는 finetuning 사용법은 난해했으나 태웅씨가 리서치한 사용법 덕분에 쉽게 학습에 성공.
- 데이터 수가 많은 행동을 선호하며, 말하기 행동은 빼먹지 않지만, 다른 논리 행동의 일부를 빼먹는 모습을 보인다.
- 안정적인 동작을 위해서는 사용하는 프롬프트 구조와 양질의 파인튜닝 데이터가 대량으로 필요할 것으로 보인다.
- 하지만 사용해야하는 문법은 틀리지 않으며, 많은 양의 데이터를 넣은 행동에 대해서는 꽤 논리적인 모습을 보인다.

### 2023.6.26
- 코드상 문제 디버깅 및 오작동되는 동작 원인 파악
- 스케쥴러와 로봇 구동 노드를 분리해서 만들었어야 했는데 급한 마음에 합쳐서 만들었더니 디버깅이 힘들다.. 반성할점.. 
- 로봇팔과 본체의 협동 동작을 만드는 작업이 쉽지 않다. 
- 로봇팔과 본체 간 정보를 교환하는 작업을 하자 교감성 상승을 위해 넣은 로봇팔의 사람 트래킹 기능이 버벅이게 된다.
- 트래킹한 로봇팔의 각도를 기준으로 회전하는 look 동작에 지장이 생길 듯 하다. 

#### 2023.6.27 ~ 28
- callback함수가 생각외로 멀티쓰레드와 비슷한 방식으로 동작하지 않아 일부 기능 구현에 어려움이 있다. follow와 stop을 위해서는 멀티쓰레드 동작이 필요할 것으로 보인다.
- 하지만 동작 도중 갱신된 정보가 반영되려면 멀티쓰레드를 어느 부분에서 넣어야 하는지 잘 모르겠다.
- timer callback을 쓰면 생각했던 기능이 멀티쓰레드처럼 만들 수 있는 것을 알았다.
- 메인 동작중 정보가 갱신되면 이를 바탕으로 timer callback으로 반영된 동작이 실행된다.
현재 follow flag, look flag, stop flag가 있으며, 그 외의 것들도 여유가 있다면 추가 가능할 것이다.
- 태웅씨가 만든 TTS 초안을 패키지화, 추가로 GPT가 요청한 음악을 재생하는 기능을 만들었다.
- 최종 조정 및 프로젝트발표 준비 
